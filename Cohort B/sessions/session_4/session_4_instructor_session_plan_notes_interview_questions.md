...
2. An `llm_classifier.py` service module that calls the OpenAI Chat Completions API with a structured system prompt requesting JSON output
...
- OpenAI Chat Completions API call (`openai` Python library, `chat.completions.create`)
- `response_format={"type": "json_object"}` (JSON mode) to enforce structured output
...
- API key loaded from environment variable (`OPENAI_API_KEY`) via `os.environ` or `python-dotenv`
- pytest tests mocking the OpenAI client with `unittest.mock.patch`
- Ollama as an optional local alternative (same OpenAI-compatible API shape)
...
Today we add the first real intelligence to the backend. When a user submits a ticket, the backend will silently call an LLM, classify that ticket, and return the classification with the response. The user gets a richer response. The support team gets structured metadata. No manual triaging needed.
...
[Save Ticket to DB] --> tickets table
     |
     v
[Call LLM Classifier] --> OpenAI API (or Ollama)
     |
...
4. Why `response_format={"type": "json_object"}`? — JSON mode forces the model to return valid JSON instead of prose, markdown, or partial JSON. Eliminates the need for regex parsing.
...
1. Open Claude Code or Cursor in the project root
...
- Is `response_format={"type": "json_object"}` present?
...
- Is `OPENAI_API_KEY` loaded from environment, not hardcoded?
...
- Point out `model="gpt-4o-mini"` (or whatever was generated) — explain why we use a smaller, faster model for classification rather than gpt-4o
- Point out `temperature=0.1` and explain the reasoning
- Point out `response_format={"type": "json_object"}` — explain this is "JSON mode"
- Show `json.loads(response.choices[0].message.content)` — this is where we parse the string response into a Python dict
...
- What happens to the route if `openai.APIConnectionError` is raised?
...
Students run the main build prompt in their own Claude Code or Cursor session and build their version of the feature.

### Setup Check Before Students Begin

Ensure every student has:

1. `OPENAI_API_KEY` set in their `.env` file (or Ollama running locally on port 11434)
...
- `openai.AuthenticationError` — means `OPENAI_API_KEY` is missing or wrong
- `ImportError: cannot import name 'OpenAI' from 'openai'` — means `openai` package is old, needs upgrade to `>=1.0.0`
...
**Scenario 1: OPENAI_API_KEY is missing or invalid**
- Expected behavior: `openai.AuthenticationError` is raised
...
**Scenario 2: LLM returns malformed JSON (rare with json_object mode, but possible)**
...
**Scenario 4: OpenAI API rate limit or timeout**
- `openai.RateLimitError` or `openai.APITimeoutError`
- Both should be caught by the broad `except Exception` or by specific exception types
- Demonstrate adding a specific `except openai.OpenAIError as e` clause
...
classify_ticket(title, description) called
     |
     v
openai.chat.completions.create() sends HTTP request to api.openai.com
     |
     v
Request body contains:
  ...
  - messages: [system prompt + user message with ticket text]
     |
     v
OpenAI returns: {"category": "...", "priority": "...", ...}
     |
     v
...
Without `response_format={"type": "json_object"}`, the model might return: "Sure! Here is the classification: `{"category": "Billing"}` ..." — wrapped in prose. `json.loads()` would fail. JSON mode guarantees the entire response is valid JSON. This eliminates an entire class of parsing failures.

**Concept 2 — Why temperature matters for classification:**
Temperature controls how much randomness the model injects during sampling. At `temperature=0`, the model always picks the most probable next token — fully deterministic. At `temperature=1`, significant randomness is introduced. For classification with a small fixed output space (e.g., 4 categories), we want consistency. `temperature=0.1` gives near-deterministic outputs while allowing the model to handle ambiguous inputs sensibly.
...
"What happens when `POST /tickets` is called and the OpenAI API is down?"
...
Today we added LLM-powered ticket classification to the backend. The feature calls OpenAI, parses structured JSON output, stores the result in a dedicated table, and degrades gracefully on failure.
...
1. **API key security** — Environment variables are not optional. `OPENAI_API_KEY` hardcoded in source code is a CVE-level mistake in a real job. Emphasize this strongly.
2. **JSON mode vs. free-text parsing** — Students often assume they can just `eval()` or regex-parse LLM output. Explain why this fails in production and why `response_format={"type": "json_object"}` is the correct solution.
...
1. **`openai.error.AuthenticationError: No API key provided`** — Student forgot to set `OPENAI_API_KEY` in `.env` or forgot to call `load_dotenv()` before the OpenAI client is initialized. Fix: ensure `load_dotenv()` is called at module import time in `app/core/config.py` or `app/main.py`.

2. **`ImportError: cannot import name 'OpenAI' from 'openai'`** — Student has `openai==0.28.x` installed (old API). The new `openai>=1.0.0` package changed the import pattern. Fix: `pip install --upgrade openai`.

3. **`json.JSONDecodeError: Expecting value`** — Student is not using `response_format={"type": "json_object"}` and the model returned prose. Fix: add `response_format={"type": "json_object"}` to the `chat.completions.create()` call and ensure the system prompt includes "respond only with JSON".
...
9. **`openai.RateLimitError` crashing the entire endpoint** — The `try/except` block is present but does not include `openai.RateLimitError`. Fix: add `except openai.OpenAIError` to catch all OpenAI-specific errors, or list them explicitly.

10. **API key committed to git** — Student added `OPENAI_API_KEY=sk-...` directly to `app/core/config.py` or similar. Fix immediately. Add `.env` to `.gitignore`. If already committed: rotate the key on the OpenAI dashboard, then remove it from git history or at minimum invalidate it.
...
If `OPENAI_API_KEY` is missing, the student switches to Ollama (if pre-installed) or follows the instructor screen and builds their own version after the session using the recorded build and the prompts from the student file.
...
The default temperature for most OpenAI model configurations is around 0.7–1.0. Temperature controls how the model samples from the probability distribution over possible next tokens. A high temperature flattens the distribution, making lower-probability tokens more likely — this increases variety but reduces consistency. For classification with a fixed, small output space (four categories, four priority levels, four sentiment values), consistency is more important than variety. Two identical tickets should always produce the same classification. At `temperature=0.1`, the model almost always picks the highest-probability token, which for a well-prompted classification task will be the correct label. Setting it to exactly 0 is also valid and fully deterministic.

### Q7. What is `response_format={"type": "json_object"}` and why is it needed?

Expected answer:

`response_format={"type": "json_object"}` is the JSON mode parameter for the OpenAI Chat Completions API. When set, the API guarantees that the model's response will be a valid JSON string — it will not include prose, markdown code fences, apologies, or explanations wrapping the JSON. Without it, the model might return a response like: "Here is the classification:\n```json\n{...}\n```" — which fails `json.loads()` because of the surrounding text. JSON mode removes this entire class of parsing failure. The requirement is that the system prompt must also mention JSON somewhere (e.g., "return a JSON object") — otherwise the API returns a validation error. Starting from newer models, `response_format={"type": "json_schema", "json_schema": {...}}` is also available for strict schema enforcement, but `json_object` is sufficient for Session 4.

### Q8. Walk through the exact sequence of DB operations in `POST /tickets` after Session 4.

Expected answer:

First, `TicketCreate` is validated by Pydantic when the request body is parsed — a `422` is returned immediately if the body is invalid. Second, a `Ticket` SQLModel instance is created from the validated data and the `current_user.id` is assigned as `user_id`. Third, `session.add(ticket)` stages the ticket for insertion. Fourth, `session.commit()` writes the ticket row to the `ticket` table and assigns it a DB-generated `id`. Fifth, `session.refresh(ticket)` loads the generated `id` back into the Python object. Sixth, `classify_ticket(ticket.title, ticket.description)` is called — this makes the HTTP request to OpenAI. Seventh, if classification succeeds, a `TicketClassification` instance is created with `ticket_id=ticket.id` and all classification fields, then `session.add(classification)` and `session.commit()` write it to the `ticket_classification` table. Eighth, the route handler builds the response object combining ticket and classification fields and returns it with HTTP 201.

### Q9. How do you write a pytest unit test for the LLM classifier when you do not want to make real API calls?

Expected answer:

We use `unittest.mock.patch` to replace the `openai.OpenAI` client or the `chat.completions.create` method with a `MagicMock` that returns a predetermined response. For example:
...
def test_classify_ticket_success():
    ...
    with patch("app.services.llm_classifier.client.chat.completions.create",
...
In Session 4, the LLM call is synchronous — the HTTP request to OpenAI blocks the FastAPI route handler until the response arrives, typically 500ms–2s. This is the simplest implementation and acceptable for a learning context. The trade-off is that high ticket creation throughput will saturate the FastAPI worker threads because each thread is blocked waiting for OpenAI. The correct production approach is to either: (a) make the LLM call async using `httpx` or the `openai` async client with `await`, which allows FastAPI's async event loop to handle other requests while waiting; or (b) push the classification to a background job queue (e.g., Celery with Redis) so the `POST /tickets` response returns immediately with a `classification_status: "pending"` field, and the classification is written to the DB when the background worker completes. The background queue approach also provides retry logic for transient LLM failures. Both are valid; the right choice depends on the SLA for classification results.

### Q12. What would you change about this design if the system needed to support multiple LLM providers (OpenAI, Ollama, Anthropic)?

Expected answer:

I would extract the LLM call behind an abstract interface — a Python `Protocol` or abstract base class `TicketClassifier` with a `classify(title: str, description: str) -> dict` method. Each provider would implement this interface: `OpenAIClassifier`, `OllamaClassifier`, `AnthropicClassifier`. The route handler would depend on the interface, not the concrete implementation. The active provider would be selected via a configuration value (e.g., `LLM_PROVIDER=openai` in `.env`). This pattern is sometimes called the Strategy pattern. It allows switching providers without touching the route or the DB logic. For Session 4, a single provider is correct because premature abstraction adds complexity without value. But in a production system with vendor risk, multi-provider support via this pattern is important.
...
- [ ] `response_format={"type": "json_object"}` is present in the OpenAI API call
- [ ] `temperature` is set to 0.1 or lower in the API call
- [ ] `OPENAI_API_KEY` is loaded from environment variable, not hardcoded in source
...
- [ ] `POST /tickets` still returns HTTP 201 when `OPENAI_API_KEY` is set to an invalid value (graceful degradation — classification fields are null)
...
- [ ] Student can explain in 2 sentences what happens when the OpenAI API is unreachable

---

# Instructor Backup Plan

If OpenAI API access fails for the majority of students (key issues, quota exhaustion, network block):

1. Switch to Ollama with a local model. If Ollama was pre-installed on student machines with `mistral` or `llama3.2` pulled, the `classify_ticket` function works without changes because Ollama exposes an OpenAI-compatible API at `http://localhost:11434/v1`. Update `base_url` in the OpenAI client initialization: `client = OpenAI(base_url="http://localhost:11434/v1", api_key="ollama")`.
2. If Ollama is not available, switch to mock mode. Create a `MOCK_LLM=true` environment variable. In `llm_classifier.py`, check this flag and return hardcoded classification JSON without making an API call. Students build the full integration code and test the DB storage and response schema, even without a live LLM.
3. Continue the code walkthrough and interview prep regardless. The concepts — JSON mode, temperature, graceful degradation, table separation — can be discussed from the code without running it.
